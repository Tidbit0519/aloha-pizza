...
describe('Pizza API', () => {
    ...
    describe('POST /api/pizzas', () => {
        it('should return an error if name or topping is missing', async () => {
            const response = await request(app).post('/api/pizzas');
            expect(response.status).toBe(400);
            expect(response.body.message).toBe('Name and toppings are required');
        });

        it('should create a mock pizza', async () => {
            const toppingObj = await request(app).post('/api/toppings').send({ name: 'mock topping' });
            const response = await request(app)
                .post('/api/pizzas')
                .send({ name: 'mock pizza', toppings: [{ _id: toppingObj.body._id }] });
            expect(response.status).toBe(201);
            expect(response.body.name).toBe('mock pizza');
        });
        ...
        it('should return an error if pizza already exists', async () => {
            ...
            expect(response.status).toBe(400);
            expect(response.body.message).toBe('Pizza already exists');
        });
    ...
    describe('PUT /api/pizzas/:id', () => {
        it('should return an error if name or toppings are missing', async () => {
            const objId = new mongoose.Types.ObjectId();
        ...
        it('should return an error if pizza already exists', async () => {
            ...
            const pizzaObj2 = await request(app).post('/api/pizzas').send({ name: 'mock pizza 2', toppings: [] });

            const response = await request(app)
                .put(`/api/pizzas/${pizzaObj1._id}`)
                .send({ name: pizzaObj2.body.name, toppings: [] });
            expect(response.status).toBe(400);
            expect(response.body.message).toBe('Pizza already exists');
        });
